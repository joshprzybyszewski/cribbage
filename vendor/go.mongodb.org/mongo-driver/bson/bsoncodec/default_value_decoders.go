...
func (dvd DefaultValueDecoders) MapDecodeValue(dc DecodeContext, vr bsonrw.ValueReader, val reflect.Value) error {
	if !val.CanSet() || val.Kind() != reflect.Map {
		return ValueDecoderError{Name: "MapDecodeValue", Kinds: []reflect.Kind{reflect.Map}, Received: val}
	}
	if !val.Type().Key().Implements(reflect.TypeOf((*fmt.Stringer)(nil)).Elem()) {
		return ValueDecoderError{Name: "MapDecodeValue2", Kinds: []reflect.Kind{reflect.Map}, Received: val}
	}
	...
	eType := val.Type().Elem()
	valDecoder, err := dc.LookupDecoder(eType)
	if err != nil {
		return err
	}
	...
	var keyDecoder ValueDecoder
	if keyType != reflect.TypeOf(``) {
		keyDecoder, err = dc.LookupDecoder(keyType)
		if err != nil {
			return err
		}
	}
	...
	for {
		...
		err = valDecoder.DecodeValue(dc, vr, elem)
		if err != nil {
			return err
		}
		...
		if keyType != reflect.TypeOf(``) {
			...
			keyVW, err := bsonrw.NewBSONValueWriter(w)
			if err != nil {
				return err
			}
			fmt.Printf("key: %+v\n", key)
			err = keyVW.WriteString(key)
			if err != nil {
				return err
			}

			keyVR := bsonrw.NewBSONValueReader(bsontype.String, b)
			fmt.Printf("keyVR.Type(): %+v\n", keyVR.Type())
			err = keyDecoder.DecodeValue(dc, keyVR, keyElem)
			if err != nil {
				return err
			}

			val.SetMapIndex(keyElem, elem)
		} else {
