...
func (dve DefaultValueEncoders) MapEncodeValue(ec EncodeContext, vw bsonrw.ValueWriter, val reflect.Value) error {
	...
	if val.Type().Key().Kind() != reflect.String && !val.Type().Key().Implements(reflect.TypeOf((*fmt.Stringer)(nil)).Elem()) {
		return ValueEncoderError{Name: "MapEncodeValue2", Kinds: []reflect.Kind{reflect.Map}, Received: val}
	}
...
func (dve DefaultValueEncoders) mapEncodeValue(ec EncodeContext, dw bsonrw.DocumentWriter, val reflect.Value, collisionFn func(string) bool) error {
	...
	valueEncoder, err := ec.LookupEncoder(val.Type().Elem())
	if err != nil {
		return err
	}
	...
	for _, key := range keys {
		...
		err = valueEncoder.EncodeValue(ec, vw, val.MapIndex(key))
		if err != nil {
			return err
		}
	}
